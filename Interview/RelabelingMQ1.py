file = open('RelabelingMQ1.txt','w+')
file.write("VARIABLE LABELS\n")


levelsVar = ["im", "ii", "bm", "bi"]
sublevelsVar = ["a", "b", "c"]
nameVar = ["ease", "num", "nsz", "nnof", "cio", "dec", "chop",  "step", "len",  "word", "key", "info", "ano", "vis", "ket", "kec", "lket", "lkec", "pat", "pac","nat", "nac", "proc", "mexp"]

levelsLabels= ["interesting mastery", "interesting instructional", "boring mastery","boring instructional"]
...
for i in range(len(nameVar)):
  for j in range(len(levelsVar)):
    for k in range(len(sublevelsVar)):
      variable = "m" + levelsVar[j] + sublevelsVar[k] + nameVar[i]
      label = "math-" + levelsLabels[j] + " #" + sublevelsLabels[k] + "-" + nameLabels[i]
...
for j in range(len(levelsVar)):
  for k in range(len(sublevelsVar)):
    variable = "m" + levelsVar[j] + sublevelsVar[k] + "ease "
    file.write(variable)
values = """
0 "no info"
1 "easiest"
